...
2. Update the library version, dependency pins and locks, README, and dated
   changelog entry.
3. Regenerate API documentation with
   `dotnet run --project tools/Goo.Animations.ApiDocs/Goo.Animations.ApiDocs.csproj -c Release`.
4. Run `bash scripts/verify.sh` and check a clean consumer of the built package.
5. Commit and push `main`, then require green CI for that commit.
6. Create and push an annotated matching `v<Version>` tag.
7. The tag workflow verifies, publishes through NuGet Trusted Publishing, and
   creates a GitHub release with the package and its checksum.
8. Confirm the version on NuGet.org and restore it in a clean package consumer.
