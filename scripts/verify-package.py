...
def validate_package(package_id, directory, license_expression, extra_files=()):
    project = ET.parse(root / "src" / package_id / f"{package_id}.gsproj").getroot()
    ...
    if not version or not re.fullmatch(r"(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)(?:-[0-9A-Za-z.-]+)?", version):
        raise SystemExit(f"{package_id} Version must be SemVer.")
    package = root / "artifacts/packages" / f"{package_id}.{version}.nupkg"
    with ZipFile(package) as archive:
        required = {f"lib/net10.0/{package_id}.dll", f"lib/net10.0/{package_id}.xml", "README.md", "LICENSE", *extra_files}
        missing = required - set(archive.namelist())
        if missing:
            raise SystemExit(f"{package_id} is missing {sorted(missing)}")
        validate_embedded_pdb(archive.read(f"lib/net10.0/{package_id}.dll"))
        manifest = ET.fromstring(archive.read(f"{package_id}.nuspec"))
        ...
        if metadata.findtext("n:license", namespaces=ns) != license_expression:
            raise SystemExit(f"{package_id} must declare {license_expression}.")
        dependencies = {item.attrib["id"]: item.attrib["version"] for item in manifest.findall(".//n:dependency", ns)}
        expected = {item.attrib["Include"]: item.attrib["Version"] for item in project.findall(".//PackageReference")}
        for item in project.findall(".//ProjectReference"):
            referenced = ET.parse(root / "src" / package_id / item.attrib["Include"]).getroot()
            expected[referenced.findtext("PropertyGroup/PackageId")] = referenced.findtext("PropertyGroup/Version")
        if dependencies != expected:
            raise SystemExit(f"Package dependencies {dependencies} do not match {expected}")
